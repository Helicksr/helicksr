...
const modelValueAsObjectArray = props.modelValue.map((tag) => ({
  label: tag,
  value: tag,
}));
...
const emits = defineEmits<{
  (eventName: typeof inputEmitName, newValue: string[]): void;
}>();

const onInput = (newValues: { label: string; value: string }[]) => {
  emits(
    inputEmitName,
    newValues.map((tagObject) => tagObject.value)
  );
};
...
const classes = {
  container:
    'relative mx-auto flex items-center justify-end box-border cursor-pointer border border-gray-300 dark:border-gray-400 dark:bg-gray-300 dark:text-black rounded-md text-base leading-snug outline-none',
  containerDisabled: 'cursor-default bg-gray-100',
  containerOpen: 'rounded-b-none',
  containerOpenTop: 'rounded-t-none',
  containerActive: 'border-indigo-300 ring ring-indigo-200 ring-opacity-50',
  wrapper:
  ...
  search:
    'w-full absolute inset-0 outline-none focus:ring-0 appearance-none box-border border-0 text-base font-sans rounded pl-3.5 rtl:pl-0 rtl:pr-3.5',
  tags: 'flex-grow flex-shrink flex flex-wrap items-center mt-1 pl-2 rtl:pl-0 rtl:pr-2',
  tag: 'bg-gray-800 text-white text-sm font-semibold py-0.5 pl-2 rounded-full mr-1 mb-1 flex items-center whitespace-nowrap rtl:pl-0 rtl:pr-2 rtl:mr-0 rtl:ml-1',
  ...
  tagsSearch:
    'absolute inset-0 border-0 outline-none focus:ring-0 appearance-none p-0 box-border w-full bg-transparent',

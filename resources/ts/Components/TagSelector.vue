...
const fetchTags = async (search: string) => {
  const response = await fetch(
    '/tags?' + new URLSearchParams({ search }),
  );
  return response.ok ? response.json() : [];
}

const classes = {
  container: 'relative mx-auto flex items-center justify-end box-border cursor-pointer border border-gray-300 dark:border-gray-400 dark:bg-gray-300 dark:text-black rounded-md text-base leading-snug outline-none',
  ...
  tagsSearch: 'absolute inset-0 border-0 outline-none focus:ring-0 appearance-none p-0 box-border w-full bg-transparent',
  ...
  groupLabelSelectedDisabled: 'text-gray-100 bg-gray-600 bg-opacity-50 cursor-not-allowed',
